      ...
      <div className="py-16 md:py-20 bg-primary-bg text-text-primary">
        <div className="container mx-auto px-6">
          <div className="text-center mb-12 md:mb-16">
            <h1 className="text-4xl md:text-5xl font-extrabold text-accent">Gallery</h1>
            <p className="text-lg md:text-xl text-text-secondary mt-4">A showcase of our work and technology in action.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {images.map((image) => (
              <motion.div 
                ...
                className="cursor-pointer overflow-hidden rounded-lg shadow-md border border-border-color" 
                whileHover={{ scale: 1.05, y: -5 }} 
                transition={{ type: 'spring', stiffness: 300 }}
              >
                <img src={image.imageUrl} alt={image.title} className="w-full h-60 object-cover" />
                <div className="p-4 bg-secondary-bg">
                  <h3 className="text-text-primary font-bold truncate">{image.title}</h3>
                </div>
              </motion.div>
            ))}
          </div>
