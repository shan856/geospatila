...
const ServicesPage = () => {
  ...
  const getIconPath = (iconName) => iconSvgPaths[iconName] || iconSvgPaths.map;

  return (
    <div className="relative bg-white pt-24">
      <SEO
        title="Services"
        description="Explore our expert geospatial services: GIS Mapping, Drone Surveying, Remote Sensing, and LiDAR Processing."
        keywords="LiDAR point cloud processing, drone photogrammetry services, satellite imagery analysis, GIS automation"
      />
      <FloatingOrbs />
              ...
              {services.map((service, index) => (
                ...
                >
                  <Card variant="elevated" className="h-full group p-0 overflow-hidden flex flex-col">
                    {/* Image Header */}
                    {/* Image Header */}
                    <Link to={`/services/${service.id}`} className="block relative h-48 overflow-hidden bg-accent/5 cursor-pointer">
                      {service.imageUrl ? (
                        <img
                        ...
                        <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-accent/5 to-geo-accent/5">
                          <svg className="w-16 h-16 text-accent/10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={getIconPath(service.icon)} />
                          </svg>
                        </div>
                      )}

                      {/* Icon overlay or fallback */}
                      <div className="absolute top-4 left-4">
                        <div className="icon-soft backdrop-blur-md bg-white/80 shadow-sm border border-white/50">
                    ...
                    <div className="p-8 flex flex-col flex-1">
                      ...
                      <div className="mt-auto pt-6 border-t border-glass-border">
                        <Link
                          ...
                          className="text-accent font-semibold inline-flex items-center gap-2 group/link hover:gap-3 transition-all"
                        >
                          Learn More
