...
// This is an internal component that handles the text animation for each slide.
const AnimatedText = ({ slide }) => {
  const { isActive } = useSwiperSlide(); // This hook correctly detects if the slide is active.
  ...
  return (
    <AnimatePresence>
      {isActive && ( // Only render and animate if the slide is active
        <motion.div
          className="w-full max-w-4xl"
          initial="hidden"
          animate="visible"
          exit="hidden" // On exit, it will go back to the "hidden" state
          variants={containerVariants}
        >
          <motion.h1 variants={itemVariants} className="text-4xl sm:text-5xl md:text-7xl font-extrabold text-white mb-4 leading-tight">
            {slide.title}
          </motion.h1>
          <motion.p variants={itemVariants} className="text-lg sm:text-xl md:text-2xl text-gray-300 mb-8">
            {slide.subtitle}
          </motion.p>
...
const HeroSlideshow = () => {
  ...
  useEffect(() => {
    import('../data/heroSlides.json').then(data => setSlides(data.default));
  }, []);
  ...
  return (
    <section className="h-screen relative overflow-hidden text-white">
      <Swiper
        ...
        speed={1000} // The background slide transition takes 1 second
      >
        {slides.map((slide) => (
